...
## Mission
The Security Auditor agent is responsible for identifying, documenting, and helping remediate security vulnerabilities across the waba-bot platform. Given the multi-tenant nature of this SaaS and its integrations with sensitive external APIs (WhatsApp Business API, Mercado Pago), the auditor must pay special attention to:
- **Data Isolation**: Ensuring tenant data cannot leak between accounts.
- **Authentication & Authorization**: Verifying that all endpoints are protected and roles are enforced.
- **Secrets Management**: Confirming that API keys, tokens, and passwords are handled securely.
- **Webhook Security**: Validating that webhook endpoints verify request signatures to prevent spoofing.
...
- `apps/` — Contains the core applications. Security-critical modules:
    - `apps/backend/src/auth/` — Authentication, JWT, Guards.
    - `apps/backend/src/payments/` — Mercado Pago integration, webhooks.
    - `apps/backend/src/waba/` — WhatsApp Business API handler.
- `coverage/` — Test coverage reports. Check for low coverage on security-critical paths.
- `implementations/` — Reference implementations; may contain legacy or experimental code.
- `packages/` — Shared types; ensure no secrets are exposed here.
...
| File | Role |
|------|------|
| [`AuthController`](apps/backend/src/auth/auth.controller.ts) | Handles login and OAuth flows. |
| [`AuthService`](apps/backend/src/auth/auth.service.ts) | Validates credentials and issues JWT tokens. |
| [`TokenService`](apps/backend/src/auth/token.service.ts) | Manages JWT signing and verification. |
| [`JwtAuthGuard`](apps/backend/src/auth/guards/jwt-auth.guard.ts) | Protects endpoints requiring authentication. |
| [`RolesGuard`](apps/backend/src/auth/guards/roles.guard.ts) | Enforces role-based access control (ADMIN, TENANT, CUSTOMER). |

### 2. Payment Processing
| File | Role |
|------|------|
| [`MercadoPagoService`](apps/backend/src/payments/mercadopago.service.ts) | Interacts with Mercado Pago API for payments. |
| [`WebhooksController`](apps/backend/src/payments/webhooks.controller.ts) | Receives payment notifications from Mercado Pago. |
| [`PaymentQueueProcessor`](apps/backend/src/payments/processors/payment-webhook.processor.ts) | Processes webhook payloads asynchronously. |

### 3. WhatsApp Business API
| File | Role |
|------|------|
| [`WabaController`](apps/backend/src/waba/waba.controller.ts) | Receives incoming messages from Meta. |
| [`WabaProcessor`](apps/backend/src/waba/waba.processor.ts) | Handles message parsing and dispatching. |

### 4. Multi-Tenant Data Access
All services interacting with Prisma must filter by `tenantId` to prevent cross-tenant data leakage. Key services to audit:
- `TenantsService`, `AppointmentsService`, `PaymentsService`, `CustomersService`, `SubscriptionsService`.
...
- [`getAuthToken`](apps/frontend/lib/api/config.ts#L39) (function)
...
- [`AuthCallbackContent`](apps/frontend/app/auth/callback/page.tsx#L7) (function)
...
- [`AuthModule`](apps/backend/src/auth/auth.module.ts#L24) (class)
- [`AuthController`](apps/backend/src/auth/auth.controller.ts#L29) (class)
- [`RolesGuard`](apps/backend/src/auth/guards/roles.guard.ts#L8) (class)
