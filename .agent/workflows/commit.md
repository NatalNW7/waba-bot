...
## Steps
1. **Pre-commit Check:** Run the `/clean` workflow to ensure there are no linting or formatting errors before committing.
...
4. **Draft Message:** Compose the message in the format: `<type>(<scope>): <description>`.
    - *Example:* `feat(api): add subscription endpoint`
    - *Example:* `fix(prisma): resolve operating hours relation bug`
